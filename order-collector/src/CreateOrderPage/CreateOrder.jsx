...
function CreateOrder() {
    let [row, setRow] = useState([{
        id: Date.now(), value: ""
    }]);//just 0 and 1 for rendering rows
    let orderStatus = ["pending", "cancelled", "completed"];
    let buttonsGroup = useRef({});
    useEffect(() => {
            ...
            <div className="create-card">
                <div className="form">
                    ...
                    <div className="btn-row">
                        <button id="sendBtn"> {SendOrderPic} <b> Create Order </b></button>
                        <div className="status-btn-container">
                            <button id="sendBtn" onClick={() => showAndHideHandler(buttonsGroup.current["status-btn-gp"])}> {MenuBtn} <b>Order Status</b> </button>
                            <div className="status-btn-gp" ref={ el => {if (el) buttonsGroup.current["status-btn-gp"] = el}}>
                                {orderStatus.map((el,index) => <button key={index}>{el}</button>)}
                            </div>
                        </div>
                    </div>
